...
export default function GamePlayer({
  ...
  /* ⭐⭐⭐ NEW — AUTO LANDSCAPE ROTATION ⭐⭐⭐ */
  useEffect(() => {
    if (!gameData) return;

    // Only run if game demands landscape
    if (gameData.orientation !== "landscape") return;

    // Only mobile/tablet (not desktop)
    const isMobileOrTablet = window.matchMedia("(max-width: 1024px)").matches;
    if (!isMobileOrTablet) return;

    // If already landscape → return
    const alreadyLandscape = window.matchMedia("(orientation: landscape)").matches;
    if (alreadyLandscape) return;

    // Try auto-rotate
    tryAutoRotate();
  }, [gameData]);

  // ⭐ Helper to request rotation (ESLint-safe)
  const tryAutoRotate = async () => {
    try {
      ...
      console.warn("Orientation change blocked:", err);
    }
  };
