...
export default function Profile() {
  ...
  const handleFilePick = (e) => {
    ...
    if (!f.type.startsWith("image/")) return setErr("Please select a valid image file.");
    if (f.size > 4 * 1024 * 1024) return setErr("Max file size: 4MB");
  ...
  const handleUpload = async () => {
    if (!file) return setErr("Please select an image first.");
    ...
    try {
      const res = await axios.post(`${API_BASE}/api/user/avatar`, fd, {
          ...
          <div style={styles.leftCol} className="profile-left">
            <div style={styles.avatarWrap} className="profile-avatar-wrap">
              <img
                ...
                alt="avatar"
                style={styles.avatar}
                onError={(e) => (e.currentTarget.src = "https://i.ibb.co/YT1y9VJ/default-avatar.png")}
              />
                ...
                <div key={g._id} style={styles.favCard} className="fav-card">
                  ...
                  <div style={styles.favInfo}>
                    ...
                    <div style={styles.favBtns}>
                      <button onClick={() => navigate(`/game/${g.slug}`)} style={styles.viewBtn}>
                        View
                      </button>

                      <button onClick={() => handleToggleFav(g._id)} style={styles.removeBtn}>
                        ♥
                      </button>
                    </div>
                  </div>
...
/* -------- RESPONSIVE FIXES (NO LOGIC CHANGED) -------- */ 
const responsive = `
