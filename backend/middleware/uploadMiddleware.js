...
import fs from "fs";
import path from "path";
import sharp from "sharp"; // ⭐ NEW: for thumbnail compression
...
export const uploadFiles = (req, res, cb) => {
  // Pehle normal multer run hoga
  baseUploader(req, res, async (err) => {
    if (err) return cb(err);

    // ⭐ EXTRA STEP: Thumbnail compress + resize + WebP
    try {
      const thumbFile = req.files?.thumbnail?.[0];

      if (thumbFile && thumbFile.path && thumbFile.filename) {
        const inputPath = thumbFile.path;
        const ext = path.extname(thumbFile.filename).toLowerCase();
        const baseName = path.basename(thumbFile.filename, ext);

        // .webp already? → same file naam, sirf compress/resize
        const outputFileName = `${baseName}.webp`;
        const outputPath =
          ext === ".webp"
            ? inputPath
            : path.join(THUMB_DIR, outputFileName);
        ...
        // Agar original PNG/JPG tha, delete it
        if (outputPath !== inputPath && fs.existsSync(inputPath)) {
          try {
            fs.unlinkSync(inputPath);
          } catch (e) {
            console.warn("⚠️ Failed to remove original thumbnail:", e.message);
          }
        }

        // ⭐ VERY IMPORTANT: Express k liye filename update karo
        thumbFile.filename = path.basename(outputPath);
        thumbFile.path = outputPath;
      }
    } catch (processErr) {
      console.error("❌ Thumbnail processing error:", processErr.message);
      // Agar compression fail ho jaye, phir bhi request ko fail na karo
      // sirf warning log hoga, original image use ho sakti hai.
    }
